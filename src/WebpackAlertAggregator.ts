...
export default class WebpackAlertAggregator implements IAlertAggregator {
  ...
  public flushAlerts(prefix: string): void {
    this.compilation.errors.push(...this.errors.map(e => new webpack.WebpackError(`${prefix}: ${e}`)))
    this.errors = []
    this.compilation.warnings.push(...this.warnings.map(w => new webpack.WebpackError(`${prefix}: ${w}`)))
    this.warnings = []
  }
}
